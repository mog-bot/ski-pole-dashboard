...
function appendDocument(state,text){const clean=text.trim();if(!clean)return;state.document.body=state.document.body.trim()?`${state.document.body.trim()}\n\n${clean}`:clean}
function projectSummary(state){const total=state.criteria.reduce((sum,item)=>sum+Number(item.weight||0),0);const met=state.success.filter(item=>item.status==='Met').length;const open=state.tasks.filter(item=>item.stage!=='Done').length;return`The project has ${state.criteria.length} scoring criteria worth ${total}% in total, ${met} of ${state.success.length} success criteria met, ${open} open tasks, ${state.research.length} research items, ${state.log.length} ideas or tests, and ${state.events.length} calendar events. Current focus: ${state.brief.focus}`}
function nextStep(state){const high=state.tasks.find(item=>item.stage!=='Done'&&item.priority==='High')||state.tasks.find(item=>item.stage!=='Done');return high?`The clearest next step is: ${high.text}. It is currently in ${high.stage}.`:'There are no open tasks. Add a testing or research task next.'}
function introduction(state){return`${state.brief.title}\n\nThis project explores ${state.brief.purpose.charAt(0).toLowerCase()+state.brief.purpose.slice(1)} The main question guiding the work is: ${state.brief.question} The current focus is ${state.brief.focus.charAt(0).toLowerCase()+state.brief.focus.slice(1)}`}
function successText(state){return`Success criteria\n\n${state.success.map((item,index)=>`${index+1}. ${item.title}\nMeasure: ${item.measure||'Not added'}\nStatus: ${item.status}${item.evidence?`\nEvidence: ${item.evidence}`:''}`).join('\n\n')}`}
...
function parseDate(text){const source=text.toLowerCase();const now=new Date();if(source.includes('day after tomorrow')){const date=new Date(now);date.setDate(date.getDate()+2);return date}if(source.includes('tomorrow')){const date=new Date(now);date.setDate(date.getDate()+1);return date}if(source.includes('today'))return now;let match=source.match(/\b(20\d{2})[-/](\d{1,2})[-/](\d{1,2})\b/);if(match)return new Date(Number(match[1]),Number(match[2])-1,Number(match[3]));const days=['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];for(let index=0;index<days.length;index++){if(source.includes(`next ${days[index]}`))return nextWeekday(index,true);if(new RegExp(`(?:this\\s+|on\\s+|for\\s+|by\\s+|to\\s+)?${days[index]}\\b`).test(source))return nextWeekday(index,false)}const months=['january','february','march','april','may','june','july','august','september','october','november','december'];match=source.match(new RegExp(`(\\d{1,2})(?:st|nd|rd|th)?(?:\\s+of)?\\s+(${months.join('|')})(?:\\s+(20\\d{2}))?`));if(match){let year=Number(match[3]||now.getFullYear());const date=new Date(year,months.indexOf(match[2]),Number(match[1]));if(!match[3]&&date<new Date(now.getFullYear(),now.getMonth(),now.getDate()))date.setFullYear(year+1);return date}return null}
...
function handleCommand(question){
  ...
  if(/summari[sz]e|project summary|how is my project/.test(lower))return bubble(projectSummary(state));
  if(/what should i do next|next step|what next/.test(lower))return bubble(nextStep(state));

  if(/what is in my (?:project )?(?:doc|document)|read my (?:doc|document)|show my (?:doc|document)/.test(lower)){const body=state.document.body.trim();return bubble(body?`${state.document.title}: ${body.slice(0,900)}${body.length>900?'…':''}`:'Your document is empty.')}
  match=question.match(/(?:set|change)\s+(?:my\s+)?(?:document|doc)\s+title\s+to\s+(.+)/i);
  if(match){state.document.title=match[1].trim();api.save();bubble(`Changed the document title to “${state.document.title}”.`,'success');return}
  match=question.match(/replace\s+(?:my\s+)?(?:document|doc)\s+with\s+(.+)/i);
  if(match){state.document.body=match[1].trim();api.save();api.showPage('document');bubble('Replaced the project document.','success');return}
  if(/write (?:an?|the) introduction (?:in|into|to) (?:my\s+)?(?:document|doc)/i.test(question)){appendDocument(state,introduction(state));api.save();api.showPage('document');bubble('Added a project introduction to the document.','success');return}
  if(/write (?:a\s+)?project summary (?:in|into|to) (?:my\s+)?(?:document|doc)/i.test(question)){appendDocument(state,projectSummary(state));api.save();api.showPage('document');bubble('Added a project summary to the document.','success');return}
  if(/write (?:my\s+)?success criteria (?:in|into|to) (?:my\s+)?(?:document|doc)/i.test(question)){appendDocument(state,successText(state));api.save();api.showPage('document');bubble('Added the success criteria to the document.','success');return}
  match=question.match(/(?:add|append|write|put)\s+(?:this\s+)?(?:paragraph|text|note)?\s*(?:to|in|into)\s+(?:my\s+)?(?:document|doc)\s*[:\-]?\s*(.+)/i);
  if(!match)match=question.match(/(?:add|append|write|put)\s+(.+?)\s+(?:to|in|into)\s+(?:my\s+)?(?:document|doc)$/i);
  if(match){appendDocument(state,match[1]);api.save();api.showPage('document');bubble('Added that to the project document.','success');return}

  if(/list (?:my )?success criteria|what are (?:my )?success criteria/.test(lower))return bubble(state.success.length?state.success.map(item=>`${item.title} [${item.status}]`).join('; '):'No success criteria are saved.');
  match=question.match(/add\s+(?:a\s+)?success criterion\s+(.+?)(?:\s+measured by\s+(.+))?$/i);
  if(match){state.success.push({id:api.makeId(),title:match[1].trim(),measure:(match[2]||'Add a measurable test using the pencil icon.').trim(),evidence:'',status:'Not started'});api.save();api.showPage('success');bubble(`Added the success criterion “${match[1].trim()}”.`,'success');return}
  match=question.match(/mark\s+(?:the\s+)?success criterion\s+(.+?)\s+as\s+(met|in progress|not started)/i)||question.match(/mark\s+(.+?)\s+as\s+(met|in progress|not started)\s+(?:in\s+)?success criteria/i);
  if(match){const item=findItem(state.success,match[1],['title']);if(!item)return bubble('I could not find that success criterion.');item.status=match[2].toLowerCase().replace(/\b\w/g,char=>char.toUpperCase());api.save();bubble(`Marked “${item.title}” as ${item.status}.`,'success');return}
  match=question.match(/add\s+evidence\s+(.+?)\s+to\s+(?:the\s+)?success criterion\s+(.+)/i);
  if(match){const item=findItem(state.success,match[2],['title']);if(!item)return bubble('I could not find that success criterion.');item.evidence=item.evidence?`${item.evidence}\n${match[1].trim()}`:match[1].trim();api.save();bubble(`Added evidence to “${item.title}”.`,'success');return}

  if(/list (?:my )?scoring criteria|what are (?:my )?scoring criteria|show scoring criteria/.test(lower))return bubble(state.criteria.length?state.criteria.map(item=>`${item.name}: ${item.weight}% (${item.status})`).join('; '):'No scoring criteria are saved.');
  match=question.match(/add\s+(?:a\s+)?(?:scoring\s+)?criterion\s+(.+?)\s+(?:with\s+)?weight\s+(\d{1,3})/i);
  if(match){const weight=Number(match[2]);if(weight>100)return bubble('A single scoring criterion cannot be more than 100%.');state.criteria.push({id:api.makeId(),name:match[1].trim(),weight,description:'Added by Project AI. Click the pencil to add judging guidance.',status:'Draft'});api.save();bubble(`Added “${match[1].trim()}” with a ${weight}% weight.`,'success');return}
  ...
  bubble('I can update success criteria, the Project Doc, scoring criteria, tasks, research, ideas, tests and the calendar. Try “Add success criterion the score is easy to explain” or “Add this paragraph to my document: …”');
}
...
function init(){api=window.ParkScoreOS;if(!api)return;$('aiToggle').addEventListener('click',()=>setPanel($('aiPanel').hidden));$('aiClose').addEventListener('click',()=>setPanel(false));$('sendAI').addEventListener('click',process);$('aiInput').addEventListener('keydown',event=>{if(event.key==='Enter')process();if(event.key==='Escape')setPanel(false)});document.querySelectorAll('[data-prompt]').forEach(button=>button.addEventListener('click',()=>{$('aiInput').value=button.dataset.prompt;process()}));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('aiPanel').hidden)setPanel(false)})}
